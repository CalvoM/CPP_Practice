...
int main(int argc, char** argv){     
    ...
    int carry{0};
    //Numbers provided as cmd arguments
    ...
    //Just thought it well to practice with iterators
    ...
    while(!stack_two.is_empty() or !stack_one.is_empty()){
        char one = stack_one.is_empty()?'0':stack_one.pop(); // check if number's digits are over; so provide 0 for summation
        char two = stack_two.is_empty() ? '0' : stack_two.pop();
        calculator(one,two,carry,stack_sum);
    }
    if(carry>0){
        stack_sum.push(carry+'0');//If carry is not zero after all additions done, contains a number to be appended to end of sum
    }
    int dash_length = str_one.size()>=str_two.size()?str_one.size():str_two.size(); //Format print
    dash_length+=2;
